...
const Introduction = () => {
  ...
  const [refH1First, inViewH1First] = useInView({ triggerOnce: true, threshold: 0.5 });
  const [refPFirst, inViewPFirst] = useInView({ triggerOnce: true, threshold: 0.5 });

  // 第二个组件的动画控制器和可视区域检测
  const controlsH1Second = useAnimation();
  const controlsPSecond = useAnimation();
  const [refH1Second, inViewH1Second] = useInView({ triggerOnce: true, threshold: 0.5 });
  const [refPSecond, inViewPSecond] = useInView({ triggerOnce: true, threshold: 0.5 });
  ...
  }, [controlsH1First, controlsPFirst, inViewH1First, inViewPFirst, controlsH1Second, controlsPSecond, inViewH1Second, inViewPSecond]);
